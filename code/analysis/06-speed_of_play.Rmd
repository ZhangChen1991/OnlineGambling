...
```{r speed of play non-parametric tests, results='asis'}
...
# next, compare the two groups
comp_between1 <- RANKT(filter(rt_wide, group == "High-Risk")$loss,
                     filter(rt_wide, group == "Low-Risk")$loss, paired = FALSE)

comp_between2 <- RANKT(filter(rt_wide, group == "High-Risk")$win,
                     filter(rt_wide, group == "Low-Risk")$win, paired = FALSE)

comp_between3 <- RANKT(filter(rt_wide, group == "High-Risk")$diff,
                     filter(rt_wide, group == "Low-Risk")$diff, paired = FALSE)
...
# next, compare the two groups
comp_between1 <- RANKT(filter(rt_z_wide, group == "High-Risk")$loss,
                     filter(rt_z_wide, group == "Low-Risk")$loss, paired = FALSE)

comp_between2 <- RANKT(filter(rt_z_wide, group == "High-Risk")$win,
                     filter(rt_z_wide, group == "Low-Risk")$win, paired = FALSE)

comp_between3 <- RANKT(filter(rt_z_wide, group == "High-Risk")$diff,
                     filter(rt_z_wide, group == "Low-Risk")$diff, paired = FALSE)
...
# show the table
kable(comp, 
      caption = "Pairwise comparisons for effects on speed of play (non-parametric)",
      booktabs = T, align = "l") %>%
  collapse_rows(columns = 1) %>%
  kable_styling(latex_options = "scale_down") %>%
  scroll_box(width = "100%", height = "100%")
